import { createProbate, createWork, deleteProbate, deleteWork, editProbate, editWork } from '@/app/helpers/api';
...
export default function AllProbates() {
    ...
    useEffect(() => {
        const getData = () => {
            axios.get(`${process.env.NEXT_PUBLIC_API}/admin/probate/all-probates`)
                .then(res => {
    ...
    const onEditData = (e) => {
        e.preventDefault();
        editProbate({ id: selectedData._id, data: inputData })
            .then(res => {
                toast.success(res.message);
                refreshData();
            })
    ...
    return (
        <div className='flex-1 flex flex-col'>
            <div className="flex justify-between items-center">
                <h1 className="text-lg font-bold">All Probates</h1>
                <button
                    onClick={() => setCurrentState('add')}
                    className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Add New Probate</button>
            </div>
            {currentState === 'view' &&
